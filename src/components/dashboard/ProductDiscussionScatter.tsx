...
import { ProductScatterData } from "@/services/api";
import {
  ResponsiveContainer,
...
interface ProductDiscussionScatterProps {
  data: ProductScatterData[];
}

export default function ProductDiscussionScatter({ data }: ProductDiscussionScatterProps) {
          ...
          <RechartsScatterChart margin={{ top: 20, right: 20, bottom: 25, left: 35 }}>
            ...
            <Scatter
              ...
              shape={(props) => {
                ...
                const product = data.find(item => item.y === props.payload.y && item.x === props.payload.x);
                return (
                  <g>
                    <circle cx={cx} cy={cy} r={8} fill={fill} fillOpacity={0.7} />
                    {product && <text x={cx} y={cy - 10} textAnchor="middle" fontSize={10}>{product.competitor}</text>}
                  </g>
                );
              }}
            />
